...
def build_crack_semi_portion_loader(split, all_cfg, seed=0, distributed=False):
    ...
    p_unsup = 1 - p_sup
    n_sup = n_sample * p_sup
    n_unsup = n_sample * p_unsup
    ld_list, ud_list = split_list(cfg_dlst, p_sup)
    # build transform
    trs_form = build_transfrom(cfg)
    trs_form_unsup = build_transfrom(cfg)

    dset = crackData(cfg["data_root"], ld_list, trs_form, seed, n_sup, split)
    ...
    if split == "val":
        ...
        # build sampler for unlabeled set
        dset_unsup = crackData(
            cfg["data_root"], ud_list, trs_form_unsup, seed, n_unsup, split
        )
...
def split_list(cfg_dlst, p_sup):
    ...
    ld_list = os.path.join(dir_path, f'labeled_portion({p_sup}/{p_unsup}).txt')
    ud_list = os.path.join(dir_path, f'unlabeled_portion({p_sup}/{p_unsup}).txt')

    with open(ld_list, 'w') as f:
        f.writelines(labeled_lines)

    with open(ud_list, 'w') as f:
        f.writelines(unlabeled_lines)

    return ld_list, ud_list